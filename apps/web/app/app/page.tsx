...
export default function AppPage() {
  ...
  // Read response as text, then try to parse JSON safely
  ...
  // Build a user-friendly error message from response body
  const buildErrorMessage = useCallback((resInfo: { status: number; json?: any; text?: string }, fallback: string) => {
    const { status, json, text } = resInfo
    const msgFromJson = json?.error || json?.message
    if (msgFromJson) return msgFromJson as string
    if (text && text.trim()) {
      const trimmed = text.trim()
      // Avoid dumping HTML into the UI
      if (trimmed.startsWith('<')) return `${fallback} (status ${status})`
      return trimmed
    }
  ...
  const fetchSetlist = useCallback(async () => {
    ...
    try {
      ...
      const info = await readJsonOrText(res)
      if (!res.ok) {
        throw new Error(buildErrorMessage(info, 'Failed to fetch setlist'))
      }
      const json = (info.json || {}) as SetlistResponse
  ...
  const createPlaylist = useCallback(async () => {
    ...
    try {
      ...
      const info = await readJsonOrText(res)
      if (!res.ok) {
        throw new Error(buildErrorMessage(info, 'Failed to create playlist'))
      }
      const json = (info.json || {}) as PlaylistResponse
      if ((json as any).error) {
        throw new Error((json as any).error)
      }
      setPlaylist(json)
